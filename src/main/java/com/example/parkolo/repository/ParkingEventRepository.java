...
public interface ParkingEventRepository extends JpaRepository<ParkingEvent, Long> {
    ...
    List<ParkingEvent> findByParkingSpotAndEventType(ParkingSpot parkingSpot, String eventType);

    List<ParkingEvent> findByTimestampBetween(LocalDateTime start, LocalDateTime end);

    @Query("SELECT e FROM ParkingEvent e WHERE e.parkingSpot.spotNumber = :spotNumber ORDER BY e.timestamp DESC")
